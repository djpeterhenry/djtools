...
def get_base_filename(filename, record):
    ...
    if ext in ['.mp3', '.flac']:
        ...
        song = ""
        try: song = audio['title'][0]
        except KeyError: pass
        if len(artist) > 0: file = '%s%s%s' % (artist, divider, song)
        elif len(song) > 0: file = song
    # add extension:
    if ext in ['.mp3', '.flac', '.als']:
        file = '%s (%s)' % (file, ext[1:].upper())
    # add vocal:
    if 'vocal' in record['tags']: file = file + ' [Vocal]'
    return file
...
def get_mp3_bpm(filename):
    ...
    bpm = None
    bpm_text = None
    try: bpm_text = audio['bpm'][0]
    except KeyError: pass
    if bpm_text is not None:
        try: bpm = int(bpm_text)
...
def get_sample_from_xml(xml_root):
    ...
    sample_path_list = [x.attrib['Dir'] for x in sample_ref.findall('FileRef/SearchHint/PathHint/RelativePathElement')]
...
def action_check_bitrate(db_filename):
    ...
    for filename, record in iter(sorted(db_dict.iteritems())):
        ...
        filepath = os.path.abspath(filename)
        try:
            sample_file = get_sample_from_alc_file(filepath) # put the try in here?
        except IOError as e:
            continue
        ...
        if 'x' in tags: continue
        if 'vocal' in tags: continue
        try:
...
def action_print_date_sample(db_filename):
    ...
    for filename, _ in iter(sorted(db_dict.iteritems())):
        ...
        try:
            ...
            date_file_tuples.append((m_time, filename))
            print ('debug: ' + sample_file + ' ' + str(m_time), file=sys.stderr)
        except OSError as e:
...
def date_list_to_dict(date_file):
    ...
    try:
        with open(date_file) as f:
            lines = f.readlines()
    except (IOError, OSError) as e: return result
    for line in lines:
...
def action_key_frequency(db_filename):
    ...
    for filename, record in iter(sorted(db_dict.iteritems())):
        ...
        if cam_key is None: continue
        key_key = reverse_camelot_dict[cam_key]
        if key_key not in key_frequency: key_frequency[key_key] = 0
...
def generate_camelot_dict():
    ...
    for k, c in initial_dict.iteritems():
        if len(k) > 1 and k[1] == 'b':
            ...
            if sharp_ascii < ord('A'): sharp_ascii = ord('G')
            sharp_key = chr(sharp_ascii)
            minor_str = ''
            if len(k) == 3: minor_str = 'm'
    ...
    for f in missing:
        ...
        if choice is not None:
            if choice == -1:
                ...
                print ('ts_list:', ts_list, 'target_ts_list:', target_ts_list, 'both_ts_list:', both_ts_list)
                # also transfer tags
